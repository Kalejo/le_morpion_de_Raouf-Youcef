...
class Board
  ...
  def victory? # méthode vérifiant si une des combinaisons gagnantes a été obtenue
    ...
    if combos[0] == "O" && combos[1] == "O" && combos[2] == "O"
      print_grid
      puts "Le joueur 2 a gagné! Congratulations !"
      exit 
    elsif combos[0] == "X" && combos[1] == "X" && combos[2] == "X"
      print_grid
      puts "Le joueur 1 a gagné! Congratulations !"
      exit
    #else
  ...
  def player_one
    ...
    print_grid
    puts "Joueur 1, STP choisi une case"
    ...
    case input 
      ...
      when "c3" 
        @c3.value = "X"
      else puts "Ce que tu viens d'écrire n'est pas valide, salop va!"
        player_one
      end
  ...
  def player_two
    ...
    print_grid
    puts "Joueur 2, STP choisi une case:"
    input = gets.chomp.downcase
    case input 
      when "a1" 
        @a1.value = "O"
...
class Game
  def initialize
    ...
    puts "Salut ! tu vas jouer au Morpion t'es content ?"
    sleep(1)
    puts "Joueur 1 écris ton nom ! allé plus vite que ça tortue !"
    @name1 = gets.chomp
    puts "Joueur 2 écris ton nom ! allé plus vite que ça tortue !"
    @name2 = gets.chomp
    puts @name1 + " joue avec les X, " + @name2 + " joue avec les 0"
    puts "Pour placer ton X ou ton 0, choisi ta cellule ( pas la cellule de prison hahaha, par exemple a1 ou b3)."
    puts "Mon pote Pour quitter la partie, fais Ctrl + c "
